...
const Select = ({ data, placeholder, icon }) => {
  ...
  const list = listData2.filter((parameter2)=>{
    return  parameter2.toLowerCase().includes(keyword);
      ...
      const list2 = list.filter((item, index,array) => array.indexOf(item) === index);
  ...
  return (
    <ClickAwayListener onClickAway={() => y(false)}>
      <div className='selecrDrop cursor position-relative'>
        {icon}
       <span className='openselect' onClick={openselect}>{selectitem.length>14 ? selectitem.subStr(0,14)+'...' : selectitem}<KeyboardDoubleArrowDownIcon className='arrow' /> </span> 
        {
          x === true &&
